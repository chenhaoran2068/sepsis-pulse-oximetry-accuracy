...
Status: private technical contract for the current paper. This is the first standardized-input module, not a claim that the entire paper can already be reproduced. It begins **after** each source database has been accessed, the eligible sepsis ICU stays identified, oxygen records selected, units and times harmonized, and event-level source and artifact rules applied. No raw database extraction or source-specific cleaning code is in this module.
...
The generic reader validates this interface, then the existing exact one-to-one pairing core independently forms 60-minute and 5-minute pairs. It does not apply the source-specific ICU selection, sepsis definition, specimen checks, unit conversions, or original event-cleaning algorithms. Later modules must separately define standardized baseline and concurrent clinical-variable inputs for the mixed-effects and random-forest analyses. The current synthetic output cannot reproduce the paper's observed estimates or final figures.
